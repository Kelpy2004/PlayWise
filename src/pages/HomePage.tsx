import { useDeferredValue, useEffect, useMemo, useState, type FormEvent, type KeyboardEvent } from 'react'
...
const FEATURES = [
  {
    ...
    description:
      'Use laptop presets or manual specs and get a practical performance readout instead of relying only on vague requirement lists.'
  },
  {
    eyebrow: 'Decision support',
    title: 'Game value and player-fit insights',
    description:
      'Every title is framed around story, gameplay, replayability, optimization, and whether it fits the kind of player you are.'
  },
  {
    eyebrow: 'Scalable foundation',
    title: 'Built for growth',
    description:
      'TypeScript, SQL-ready APIs, analytics hooks, and reusable components make PlayWise easier to maintain, expand, and polish over time.'
  }
] as const
...
export default function HomePage() {
  ...
  const [catalogStatus, setCatalogStatus] = useState({ loading: false, message: '' })
  ...
  const openSourceGames = useMemo(() => getOpenSourceGames(), [])
  const spotlightSourceGames = useMemo(
    () => featuredGames.filter((game) => !game.openSource),
    [featuredGames]
  )
  const [spotlightPool, setSpotlightPool] = useState<GameRecord[]>(() =>
  ...
  const externalCatalogCount = useMemo(
    () => allGames.filter((game) => game.catalogSource === 'igdb').length,
    [allGames]
  )
  const {
  ...
  const heroStats = useMemo(
    ...
    [allGames.length, openSourceGames.length]
  )
  ...
  useEffect(() => {
    ...
    setSpotlightIndex(0)
  }, [featuredGames, spotlightSourceGames])

  useEffect(() => {
    let ignore = false

    async function loadCatalogFromApi() {
      setCatalogStatus({ loading: true, message: '' })
      ...
      try {
        ...
        if (!ignore) {
          setCatalogStatus({
            loading: false,
            message: error instanceof Error ? error.message : 'Expanded catalog could not be loaded right now.'
          })
        }
      } finally {
        if (!ignore) {
          setCatalogStatus((current) => ({ ...current, loading: false }))
        }
      }
    }
  ...
  async function handleContactSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setContactStatus({ tone: 'danger', message: error instanceof Error ? error.message : 'Could not send the message.' })
    } finally {
      setIsSending(false)
    }
  }
  ...
  function handleSpotlightOpen() {
    if (spotlightGame?.slug) {
      void trackEvent(
        {
          category: 'discovery',
          action: 'spotlight_opened',
          label: spotlightGame.slug
        },
        token
      )
  ...
  function handleSpotlightKeyDown(event: KeyboardEvent<HTMLElement>) {
        ...
        <div className="container">
          <div className="section-banner home-catalog-banner">
            <div>
              ...
              {externalCatalogCount ? (
                <p className="text-secondary-emphasis mb-0 mt-3">
                  {externalCatalogCount} additional top-rated titles are being pulled in from the live IGDB catalog.
                </p>
              ) : null}
              {catalogStatus.message ? (
                <div className="alert alert-warning rounded-4 mt-3 mb-0 py-2 px-3">{catalogStatus.message}</div>
              ) : null}
              <div className="home-catalog-actions mt-3">
                <button type="button" className="btn btn-outline-dark rounded-pill px-4" onClick={handleWishlistJump}>
                  Wishlist
                </button>
                <span className="badge rounded-pill text-bg-light">{wishlistGames.length} saved</span>
              </div>
              {wishlistStatus.message ? (
                <div className={`alert alert-${wishlistStatus.tone} rounded-4 mt-3 mb-0 py-2 px-3 home-toast-alert`}>
                  {wishlistStatus.message}
                </div>
              ) : null}
            </div>
            <div className="home-catalog-summary">
              ...
              <span>{activeSearch ? `matching "${activeSearch}"` : catalogStatus.loading ? 'refreshing catalog…' : 'games in the active catalog'}</span>
            </div>
          </div>
        </div>
      </section>

      <section id="wishlist" className="pb-5 anchor-section">
        <div className="container">
          <div className="section-banner wishlist-banner">
            <div>
              <p className="eyebrow text-uppercase mb-2">Wishlist</p>
              <h2 className="h3 mb-2">Your saved games stay together here.</h2>
              <p className="text-secondary-emphasis mb-0">
                Tap the heart on any game card to add it here and keep track of what you want to play later.
              </p>
            </div>
            <div className="home-catalog-summary">
              <strong>{wishlistGames.length}</strong>
              <span>{wishlistGames.length ? 'games in your wishlist' : 'wishlist is empty'}</span>
            </div>
          </div>
          ...
          {wishlistGames.length ? (
            <div className="row g-4 mt-1">
              {wishlistGames.map((game) => (
                <div key={`wishlist-${game.slug}`} className="col-md-6 col-xl-4">
                  <GameCard
                    game={game}
            ...
            <div className="feature-card empty-state-card mt-3">
              <p className="eyebrow text-uppercase mb-2">Nothing saved yet</p>
              <h2 className="h4 mb-2">Start hearting game cards to build your wishlist.</h2>
              <p className="text-secondary-emphasis mb-0">
                Your wishlist is account-based, so you can come back later and keep deciding from where you left off.
              </p>
            </div>
          )}
        </div>
